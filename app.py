...
# --- 1. 系統初始化 ---
st.set_page_config(page_title="港股 SMA 矩陣分析系統 (Pro)", page_icon="📈", layout="wide")
...
def simulate_bs_data(df, tsi):
    ...
    # 定義模擬權重 (基於成交量拆分)
    ubtb = vol * 0.15 # 超大買 15%
    btb  = vol * 0.25 # 大買 25%
    rib  = vol * 0.10 # 散買 10%
    
    ubts = vol * 0.15 # 超大賣 15%
    bts  = vol * 0.25 # 大賣 25%
    ris  = vol * 0.10 # 散賣 10%

    # MMB = (UBTB*0.9 + BTB*0.7) / AvgP / TSI * 100
    df['MMB'] = (ubtb*0.9 + btb*0.7) / avg_p / tsi * 100
    
    # RTB = (UBTB*0.1 + BTB*0.3 + RIB) / AvgP / TSI * 100
    df['RTB'] = (ubtb*0.1 + btb*0.3 + rib) / avg_p / tsi * 100

    # MMS = (UBTS*0.1 + BTS*0.7) / AvgP / TSI * 100
    # 根據Code 2註釋，MMS公式與圖片可能存在差異，此處保留Code 2邏輯
    df['MMS'] = (ubts*0.1 + bts*0.7) / avg_p / tsi * 100

    # RTS = (UBTS*0.1 + BTS*0.3 + RIS) / AvgP / TSI * 100
    # 修正了Code 2中可能的變量筆誤 (BTB -> BTS) 以符合賣盤邏輯
    df['RTS'] = (ubts*0.1 + bts*0.3 + ris) / avg_p / tsi * 100
...
with st.sidebar:
    ...
    with st.expander("✈️ Telegram 分析與發送", expanded=True):
        def_token = st.secrets.get("telegram", {}).get("token", "")
        def_chat_id = st.secrets.get("telegram", {}).get("chat_id", "")
...
if not current_code:
    st.title("港股 SMA 矩陣分析 v9.7")
    st.info("👈 請從左側輸入代號或選擇收藏股票。")
else:
    ...
    if df is not None and not df.empty and shares_outstanding is None:
        st.warning("⚠️ 無法自動獲取流通股數 (TSI)，請手動輸入以啟用完整換手率與買賣盤分析。")
        ...
        if len(df) < req_len:
           ...
           # 2. SMA Matrix
            ...
            for p in matrix_intervals:
                ...
                matrix_data[p] = {
                    "max": val_max, "min": val_min, "sma": val_curr, "smac": smac_val
                }
            ...
            avg_avg_p = sum(avgp_vals) / len(avgp_vals) if len(avgp_vals) > 0 else 0
            
            avgp_mr_vals = []
            for v in avgp_vals:
                mr = (v / avg_avg_p) - 1 if avg_avg_p != 0 else 0
                avgp_mr_vals.append(mr * 100)
            avg_avgp_mr_total = sum(avgp_mr_vals) / len(avgp_mr_vals) if avgp_mr_vals else 0
            ...
            amp_rolling_vals = [] 
            for p in matrix_intervals:
                val = df['AMP'].rolling(p).mean().iloc[-1] if len(df) >= p else 0
            ...
            amp_mr_vals = []
            if avg_amp != 0:
                amp_mr_vals.append((val_amp0 / avg_amp - 1) * 100) # MR0
                for v in amp_rolling_vals:
                    amp_mr_vals.append((v / avg_amp - 1) * 100)
            else:
                amp_mr_vals = [0] * (len(amp_rolling_vals) + 1)

            avg_amp_mr_total = sum(amp_mr_vals) / len(amp_mr_vals) if amp_mr_vals else 0

            # --- Render Price Interface Table ---
            ...
            # 4. Turnover & Buy/Sell Interface (Merged Logic)
            st.subheader("📋 Turnover & Buy/Sell Matrix")
            if not has_turnover:
                st.error("⚠️ 無流通股數 (TSI) 數據，無法顯示換手率與買賣盤模擬矩陣。")
            else:
                # --- Turnover Data ---
                ...
                # 準備 Interval Sum 數據
                sum_mmb = [f"{df['MMB'].tail(p).sum():.4f}%" for p in intervals_tor]
                sum_mms = [f"{df['MMS'].tail(p).sum():.4f}%" for p in intervals_tor]
                sum_rtb = [f"{df['RTB'].tail(p).sum():.4f}%" for p in intervals_tor]
                sum_rts = [f"{df['RTS'].tail(p).sum():.4f}%" for p in intervals_tor]

                # --- Render Turnover + Buy/Sell Table ---
                ...
                tor_html += f'<tr><td>{vals_d8_d13[0]}</td><td>{vals_d8_d13[1]}</td><td>{vals_d8_d13[2]}</td><td>{vals_d8_d13[3]}</td><td>{vals_d8_d13[4]}</td><td>{vals_d8_d13[5]}</td></tr></table>'
                
                # Metrics Table (Turnover + B/S)
                tor_html += '<table class="big-font-table" style="margin-top: 10px;">'
                tor_html += '<tr style="background-color: #ffe0b2;"><th>Metrics</th>' + "".join([f"<th>Int: {p}</th>" for p in intervals_tor]) + '</tr>'
                tor_html += f'<tr><td><b>Sum(TOR)</b></td>' + "".join([f"<td>{v}</td>" for v in sums]) + '</tr>'
                
                # Buy/Sell Rows
                tor_html += f'<tr><td style="color:#d9534f; font-weight:bold;">Sum(MMB)</td>' + "".join([f"<td>{v}</td>" for v in sum_mmb]) + '</tr>'
                tor_html += f'<tr><td style="color:#28a745; font-weight:bold;">Sum(MMS)</td>' + "".join([f"<td>{v}</td>" for v in sum_mms]) + '</tr>'
                tor_html += f'<tr><td>Sum(RTB)</td>' + "".join([f"<td>{v}</td>" for v in sum_rtb]) + '</tr>'
                tor_html += f'<tr><td>Sum(RTS)</td>' + "".join([f"<td>{v}</td>" for v in sum_rts]) + '</tr>'
                
                # Turnover Stats
                tor_html += f'<tr><td><b>Max(TOR)</b></td>' + "".join([f"<td>{v}</td>" for v in maxs]) + '</tr>'
                tor_html += f'<tr><td><b>Min(TOR)</b></td>' + "".join([f"<td>{v}</td>" for v in mins]) + '</tr>'
                tor_html += f'<tr><td><b>AVGTOR</b></td>' + "".join([f"<td>{v}</td>" for v in avgs]) + '</tr>'
                tor_html += '</table>'
                
                # B/S Day View
                tor_html += '<table class="big-font-table" style="margin-top: 10px;">'
                tor_html += '<tr class="header-row"><td>B/S Simulation</td><td>Day 0</td><td>Day 1</td><td>Day 2</td><td>Day 3</td><td>Day 4</td><td>Day 5</td><td>Day 6</td></tr>'
                for name, data in [("MMB (%)", days_mmb), ("MMS (%)", days_mms), ("RTB (%)", days_rtb), ("RTS (%)", days_rts)]:
                     tor_html += f'<tr class="data-row"><td style="background-color:#fff !important; font-weight:bold;">{name}</td>' + "".join([f"<td>{d}</td>" for d in data]) + '</tr>'
                tor_html += '</table>'

                tor_html += f'<div style="margin-top:5px; font-size:12px; color:grey;">AVGTOR 7 (Total Avg): {avg_tor_7}</div>'
                st.markdown(tor_html, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown("### 📚 歷史功能與圖表")
    
    tab1, tab2, tab3, tab4 = st.tabs(["📉 Price & SMA", "🔄 Ratio Curves", "📊 Volume (Abs)", "💹 Turnover & B/S"])
    ...
    with tab4:
        if has_turnover: 
            st.line_chart(display_df[['Turnover_Rate', 'MMB', 'MMS']])
        else:
            st.info("無數據")
